const TEXT_STYLES_MAPPER = {
  ...
  color: (value) =>
    `color: rgba(${value.r * 255}, ${value.g * 255}, ${value.b * 255}, ${
      value.a
    });`,
...
const BLOCK_STYLES_MAPPER = {
  backgroundColor: (value) =>
    `background-color: rgba(${value.r * 255}, ${value.g * 255}, ${
      value.b * 255
    }, ${value.a});`,
  strokeWeight: (value) => `border-width: ${value}px;`,
  borderColor: (value) =>
    `border-color: rgba(${value.r * 255}, ${value.g * 255}, ${value.b * 255}, ${
      value.a
    });`,
  ...
  counterAxisAlignItems: (value) =>
    `display: flex; align-items: ${value.toLowerCase().replace("_", "-")};`,
  primaryAxisAlignItems: (value) =>
    `display: flex; justify-content: ${value.toLowerCase().replace("_", "-")};`,
};
const EFFECTS_MAPPER = {
  DROP_SHADOW: (value) =>
    `box-shadow: ${value.offset?.x}px ${value.offset?.y}px ${
      value.radius
    }px rgba(${value.color?.r * 255}, ${value.color?.g * 255}, ${
      value.color?.b * 255
    }, ${value.color?.a});`,
};
...
const getParentStyles = (node, currentIndex, nodeParent) => {
  ...
  for (let [key, value] of Object.entries(nodeParent))
    if (PARENT_STYLES_MAPPER[key])
      styleArr.push(
        PARENT_STYLES_MAPPER[key](value, node, currentIndex, nodeParent)
      );
  return styleArr.join(" ");
};
...
const getTextStyles = (node) => {
  ...
  if (node.absoluteBoundingBox)
    for (let [key, value] of Object.entries(node.absoluteBoundingBox))
      if (BOUNDING_STYLES_MAPPER[key])
        styleArr.push(BOUNDING_STYLES_MAPPER[key](value));
...
const getRectanglesStyles = (node) => {
  ...
  if (node.fills?.length)
    for (let [key, value] of Object.entries(node.fills[0]))
      if (RECTANGLE_STYLES_MAPPER[key])
        styleArr.push(RECTANGLE_STYLES_MAPPER[key](value));
  return styleArr.join(" ");
};
...
const getChildren = (childrens, nodeParent) => {
  return childrens?.length
    ? childrens.map((el, i) => getHTML(el, i, nodeParent)).join("")
    : "";
};
...
const PRIMITIVES = {
  ...
  RECTANGLE: (node, currentIndex, nodeParent) => {
    ...
    return buildBlock({
      ...
      className: node.type,
      style:
        parentStyles + rectStyles + currentStyles + "box-sizing: border-box;",
    });
  },
};
